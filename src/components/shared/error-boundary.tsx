  ...
  return (
    <div className={className}>
      {Array.from({ length: lines }).map((_, i) => (
        <div
          key={i}
          className="mb-3 h-3 animate-pulse rounded bg-[var(--border-default)]"
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-4">
      <div className="mb-3 flex items-center gap-3">
        <div className="h-10 w-10 animate-pulse rounded-lg bg-[var(--border-default)]" />
        <div className="flex-1">
          <div className="mb-2 h-3 w-3/4 animate-pulse rounded bg-[var(--border-default)]" />
          <div className="h-2 w-1/2 animate-pulse rounded bg-[var(--border-default)]" />
        </div>
      </div>
...
export function EmptyState({ icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex min-h-[300px] flex-col items-center justify-center py-12 text-center">
      {icon && <div className="mb-4 text-[var(--text-tertiary)] opacity-40">{icon}</div>}
      <h3 className="mb-1 text-sm font-bold text-muted-foreground">{title}</h3>
      {description && <p className="mb-5 max-w-xs text-xs text-[var(--text-tertiary)]">{description}</p>}
      {action}
    </div>
  );
}
