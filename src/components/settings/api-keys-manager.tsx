...
export function ApiKeysManager({
    ...
    return (
      <Card className="p-8 text-center">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-border bg-background">
          <svg className="text-[var(--text-tertiary)]" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
  ...
  return (
    <div>
      ...
      {createdKey && (
        <div className="mb-5 rounded-xl border border-[var(--success)]/20 bg-[var(--success)]/[0.04] p-4">
          <div className="mb-2 text-sm font-bold text-[var(--success)]">API key created</div>
          <p className="mb-3 text-xs text-muted-foreground">
            Copy this key now. You won&apos;t be able to see it again.
          </p>
          <div className="flex gap-2">
            <code className="flex-1 rounded-lg border border-border bg-white px-3 py-2 font-mono text-xs text-foreground">
              {createdKey}
            </code>
      ...
      {/* Create form */}
      {showCreate && (
        <div className="mb-5 flex gap-2 rounded-xl border border-border bg-background p-3">
          <input
            type="text"
            value={newKeyName}
      ...
      {keys.length === 0 ? (
        <Card className="p-6 text-center">
          <p className="text-sm text-[var(--text-tertiary)]">No API keys yet. Create one to get started.</p>
        </Card>
      ) : (
          ...
          {keys.map((key) => (
            <Card key={key.id} className="flex items-center gap-4 p-4">
              <div className="flex-1">
                ...
                <div className="mt-0.5 flex items-center gap-3 font-mono text-xs text-[var(--text-tertiary)]">
              ...
              {key.isActive && (
                <Button
                  ...
                  className="text-[var(--error)] hover:bg-[var(--error)]/[0.04]"
                >
                  Revoke
                </Button>
              )}
      ...
      {/* Docs link */}
      <div className="mt-6 rounded-xl border border-border bg-white p-4 text-center">
        <p className="text-sm text-muted-foreground">
          Need help integrating?{' '}
          <a href="/docs/api" className="font-semibold text-[var(--accent-primary)] hover:underline">
            Read the API docs →
          </a>
        </p>
      </div>
